...
def load_api_params(tf_config, graph, api_config="../config/code2nl.yaml"):
    params = {}
...
#        config.score_functions = evaluation.name_mapping[config.score_function]
    ...
    with tf.Session(config=tf_config, graph=transfer_graph) as sess:
        ...
        # writer = tf.summary.FileWriter("/home/bohong/文档/seqmodel/TL-CodeSum/logs", sess.graph)
        if args.save:
            model.save()
            ...
            # code = "public static DescriptorExtensionList < NodeMonitor , Descriptor < NodeMonitor > > all ( ) { return Jenkins . getInstance ( ) . < NodeMonitor , Descriptor < NodeMonitor > > getDescriptorList ( NodeMonitor . class ) ; } "
            # api = "Jenkins.getInstance"
            # model.translate(code,api)
